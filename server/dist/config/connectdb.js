...
const {
  ...
  // Required for Cloud SQL Connector
  CLOUD_SQL_IP_TYPE // Optional ('PUBLIC', 'PRIVATE', 'PSC')
} = process.env;

// --- Validation ---
if (!DB_USER || !DB_NAME || !CLOUD_SQL_INSTANCE_CONNECTION_NAME) {
  console.error("❌ Missing required environment variables: DB_USER, DB_NAME, CLOUD_SQL_INSTANCE_CONNECTION_NAME");
  ...
  while (retries > 0 && !connectionSuccess) {
    try {
      ...
      pool = mysql.createPool({
        // Assign to the outer 'pool' variable
        ...clientOpts,
        user: DB_USER,
