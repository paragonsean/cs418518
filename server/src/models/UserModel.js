...
class UserModel {
  ...
  static async updateUserProfile(userId, { firstName, lastName }) {
    try {
      ...
      if (result.affectedRows > 0) {
        ...
        logger.warn(`⚠️ No changes made to profile - User ID: ${userId}`);
  ...
  static async verifyUserEmail(email) {
    try {
      await pool.execute("UPDATE user SET is_verified = ?, verification_token = NULL WHERE u_email = ?", [true, email]);
