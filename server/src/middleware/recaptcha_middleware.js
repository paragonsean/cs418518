...
export function recaptcha(actionName) {
  return async function (req, res, next) {
    ...
    if (!token) {
      return res.status(400).json({ status: "failed", message: "Missing reCAPTCHA token" });
    }
    ...
    try {
      ...
      if (!result.success || result.action !== actionName || result.score < MIN_SCORE) {
        logger.warn(
          `reCAPTCHA (${actionName}) failed — action: ${result.action}, score: ${result.score}`
        );
      ...
      return res.status(500).json({ status: "failed", message: "reCAPTCHA error" });
    }
  };
}
