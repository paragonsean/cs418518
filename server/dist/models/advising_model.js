...
class AdvisingModel {
  ...
  static async getRecordsByEmail(studentEmail) {
    try {
      const [rows] = await executeQuery("SELECT * FROM courseadvising WHERE student_email = ? ORDER BY date DESC", [studentEmail]);
      if (rows.length === 0) {
  ...
  static async createAdvisingRecord({
    ...
    try {
      ...
      const [result] = await executeQuery(`INSERT INTO courseadvising 
  ...
  static async updateRecordById(id, data) {
    try {
      ...
      const [result] = await executeQuery(`UPDATE courseadvising 
       SET date = ?, 
           current_term = ?, 
           last_term = ?, 
           last_gpa = ?, 
  ...
  static async updateStatusById(id, status, rejectionReason = "N/A") {
    try {
      const [result] = await executeQuery("UPDATE courseadvising SET status=?, rejectionReason=? WHERE id=?", [status, rejectionReason, id]);
  ...
  static async getRecordById(id) {
    try {
      const [rows] = await executeQuery("SELECT * FROM courseadvising WHERE id = ?", [id]);
      if (rows.length === 0) {
