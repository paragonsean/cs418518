...
class CoursesModel {
  ...
  static async getCourseByLevel(level) {
    try {
      ...
      logger.error(` Error retrieving course for level ${level}: ${error.message}`);
      throw error;
    }
  }
  ...
  static async updateCourseName(level, courseName) {
    try {
      const [result] = await executeQuery("UPDATE courses SET course_name=? WHERE course_level=?", [courseName, level]);
      ...
      logger.error(` Error updating course name for level ${level}: ${error.message}`);
      throw error;
    }
  }
  ...
  static async updateCoursePrerequisite(level, prerequisite) {
    try {
      const [result] = await executeQuery("UPDATE courses SET prerequisite=? WHERE course_level=?", [prerequisite, level]);
      if (result.affectedRows === 0) {
      ...
      logger.error(` Error updating prerequisite for level ${level}: ${error.message}`);
      throw error;
    }
  }
  ...
  static async addCourse({
    ...
    try {
      const [result] = await executeQuery("INSERT INTO courses (course_name, course_level, prerequisite, course_lvlGroup) VALUES (?, ?, ?, ?)", [course_name, course_level, prerequisite, course_lvlGroup]);
      logger.info(` New course added: ${course_name} (Level: ${course_level})`);
      return {
        id: result.insertId
      };
    } catch (error) {
      logger.error(` Error inserting course (${course_name}): ${error.message}`);
      throw error;
    }
  }
  ...
  static async deleteCourse(level) {
    try {
      ...
      logger.error(` Error deleting course for level ${level}: ${error.message}`);
      throw error;
    }
  }
}
