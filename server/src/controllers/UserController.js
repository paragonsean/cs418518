...
class UserController {
  ...
  static async changeUserPassword(req, res) {
    ...
    if (!password || !password_confirmation) {
      logger.warn(`Password change failed - Missing fields (User ID: ${req.user?.userId || "unknown"})`);
      return res.status(400).json({ status: "failed", message: "All fields are required" });
    }
    
    if (password !== password_confirmation) {
      logger.warn(`Password change failed - Passwords do not match (User ID: ${req.user?.userId || "unknown"})`);
    ...
    try {
      ...
      logger.error(`Error changing password for User ID ${req.user?.userId || "unknown"}: ${error.message}`);
  ...
  static async loggedUser(req, res) {
    try {
      ...
      if (!userId) {
        logger.warn("⚠️ No user ID found in request.");
        return res.status(400).json({ status: "failed", message: "Invalid request" });
      }

      const user = await UserModel.findByUserId(userId);
      if (!user) {
        logger.warn(`⚠️ User not found - User ID: ${userId}`);
      ...
      logger.error(`Error retrieving user profile (User ID: ${req.user?.userId || "unknown"}): ${error.message}`);
      return res.status(500).json({ status: "failed", message: "Server error" });
    }
  }
  ...
  static async updateUserProfile(req, res) {
    try {
      ...
      if (!firstName || !lastName) {
        logger.warn(`⚠️ Profile update failed - Missing fields (User ID: ${userId})`);
        return res.status(400).json({ status: "failed", message: "First Name and Last Name are required" });
      }

      const updateResult = await UserModel.updateUserProfile(userId, { firstName, lastName });
