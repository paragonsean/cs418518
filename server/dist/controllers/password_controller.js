import { hashPassword, verifyToken, generateToken } from "../services/auth_service.js";
...
class PasswordController {
  ...
  static async verifyEmail(req, res) {
    ...
    const {
      recaptchaToken
    } = req.body; // reCAPTCHA token from frontend
    ...
    try {
      ...
      if (!user) {
        logger.warn(`Email verification failed - No user found (email: ${email})`);
        return res.status(400).json({
          status: "failed",
          message: "Invalid or expired token"
        });
  ...
  static async sendUserPasswordResetEmail(req, res) {
    const {
      email,
      recaptchaToken
    } = req.body; // Accept reCAPTCHA token
    ...
    try {
      ...
      const user = await UserModel.findByEmail(email);
      if (!user) {
        logger.warn(`Password reset request failed - No user found (email: ${email})`);
        return res.status(400).json({
          status: "failed",
  ...
  static async resetPassword(req, res) {
    ...
    try {
      ...
      logger.error(`Error resetting password for ${decoded.email}: ${error.message}`);
      return res.status(400).json({
        status: "failed",
        message: "Invalid or expired token"
      });
